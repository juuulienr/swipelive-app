<template>
  <main class="my_profile1" style="padding: 0px 15px 15px;">
    <div class="checkout__header" style="padding: 5px 5px 30px 5px;">
    ...
    <div v-if="user" class="top-author--container" style="padding: 0px;">
      <div class="top-author--item">
        <img v-if="user.picture" :src="cloudinary256x256 + user.picture" class="user" style="margin: 5px; width: 80px; height: 80px;">
        <img v-else class="user" :src="require(`@/assets/img/anonyme.jpg`)" style="margin: 5px; width: 80px; height: 80px;">
        <div>
        ...
        <span v-else style="font-size: 16px; font-weight: 500;">{{ user.firstname }} {{ user.lastname }}
          ...
          <div>
            <span v-if="user.vendor">{{user.followers.length }} abonnés</span>
          </div>
        </div>
    ...
    <div v-if="user" class="info_profile">
      <div v-if="user.vendor" @click="goPrelive()" class="btn-follow">
        <div class="btn-swipe" style="color: white; text-align: center; width: fit-content; background: #ff2773; margin: 0px auto; padding: 14px 48px; border: 1px solid #ff2773; border-radius: 16px; margin-bottom: 25px; font-weight: 500">Lancer un live</div>
      </div>
      <div v-if="user.vendor" class="profile--follow" style="border: 1px solid rgba(22, 24, 35, 0.12); padding: 12px 0px;">
        <div>
          <h4 v-if="user.vendor.products.length > 1">{{ user.vendor.products.length }}</h4>
          <h4 v-else>-</h4>
          <p v-if="user.vendor.products.length > 1">Articles</p>
          <p v-else>Article</p>
        </div>
        <hr>
        <div>
          <h4 v-if="user.vendor.lives.length > 1">{{ user.vendor.lives.length }}</h4>
          <h4 v-else>-</h4>
        ...
        <hr>
        <div>
          <h4 v-if="user.vendor.sales.length > 1">{{ user.vendor.sales.length }}</h4>
          <h4 v-else>-</h4>
      ...
      <li v-if="user.vendor" style="border: 1px solid rgba(22, 24, 35, 0.12); border-radius: 16px; margin: 12px auto;">
        <router-link :to="{ name: 'ShopVendor' }">
          <span style="font-size: 16px; color: #333;">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 616 512" style="width: 28px; height: 28px; margin-right: 12px;"><path d="M547.69,286.8a102.1,102.1,0,0,0,16.4-3.6V480a32,32,0,0,1-32,32H84a32,32,0,0,1-32-32V283.2a125.89,125.89,0,0,0,16.4,3.6,135.49,135.49,0,0,0,18,1.2,132.81,132.81,0,0,0,29.51-3.8V384H500V284.2a127.12,127.12,0,0,0,29.51,3.8A138.38,138.38,0,0,0,547.69,286.8Z" class="fa-secondary"></path><path d="M602,118.6c33.6,53.6,3.8,128-59,136.4a102.9,102.9,0,0,1-13.7.9,99.08,99.08,0,0,1-73.81-33.1A98.83,98.83,0,0,1,316,230.88a96.26,96.26,0,0,1-8.08-8.08,98.9,98.9,0,0,1-139.62,8,97.4,97.4,0,0,1-8-8,98.75,98.75,0,0,1-73.81,33.1,104.6,104.6,0,0,1-13.7-.9C10.12,246.5-19.58,172.1,14,118.6L78.83,15A32,32,0,0,1,106,0H510a32,32,0,0,1,27.07,15Z" class="fa-primary"></path></svg>
          </span>
