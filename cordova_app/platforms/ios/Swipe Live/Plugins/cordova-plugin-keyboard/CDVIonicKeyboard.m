...
// Ajout de la méthode shrinkView dans la classe CDVIonicKeyboard
...
- (void)shrinkView:(CDVInvokedUrlCommand*)command {
    ...
    id value = [command.arguments objectAtIndex:0];
    BOOL shouldShrink = [value boolValue];  // Valeur passée depuis JS (true ou false)

    if (shouldShrink) {
        // Réduire la vue lorsque le clavier est affiché
        self.keyboardResizes = ResizeIonic;  // Utilise le mode de redimensionnement "ionic" pour réduire la vue
    } else {
        // Ne pas réduire la vue
        self.keyboardResizes = ResizeNone;
    }

    // Mettre à jour la frame pour refléter immédiatement ce changement
    [self _updateFrame];
    
    // Envoyer un résultat de succès au callback JS
